...
#ifdef FOR_2076
	bt_lpm.gpio_reset_bt = acpi_get_gpio_by_index(&pdev->dev,
						gpio_reset_bt_acpi_idx, &info);
	if (!gpio_is_valid(bt_lpm.gpio_reset_bt)) {
		pr_err("%s: gpio %d for gpio_reset_bt not valid\n", __func__,
...
static int bcm43xx_bt_rfkill_set_power(void *data, bool blocked)
{
	...
	if (!blocked) {
		...
		*/
		...
		pr_debug("%s: turn BT on - GPS_Reset \n", __func__);
#endif
	} else {

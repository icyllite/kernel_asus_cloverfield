...
//=============================================================================================================
	...
	//----[HX_FW_UPDATE_BY_I_FILE]--------------------------------------------------------------------------start
		...
		static bool i_update_func_success = false;
		static int i_update_max_times = 3;
		static int i_update_count = 1;
...
//=============================================================================================================
		...
		static uint8_t himax_calculateChecksum(char *ImageBuffer, int fullLength)//, int address, int RST)
		{
			...
			else if(IC_CHECKSUM == HX_TP_BIN_CHECKSUM_CRC)
			{
				...
				if(IC_TYPE == HX_85XX_E_SERIES_PWON)
				{
					...
					printk("Himax check 0xED : cmd[0] = %d,cmd[1] = %d,cmd[2] = %d,cmd[3] = %d,cmd[4] = %d. \n",cmd[0],cmd[1],cmd[2],cmd[3],cmd[4]);
					...
					//For check 0x43
					if( i2c_himax_read(touch_i2c, 0x43, cmd, 5, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return -1;
					}
					printk("Himax check 0x43 : cmd[0] = %d,cmd[1] = %d,cmd[2] = %d,cmd[3] = %d,cmd[4] = %d. \n",cmd[0],cmd[1],cmd[2],cmd[3],cmd[4]);
					...
					//For check 0xD2
					if( i2c_himax_read(touch_i2c, 0xD2, cmd, 5, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return -1;
					}
					printk("Himax check 0xD2 : cmd[0] = %d,cmd[1] = %d,cmd[2] = %d,cmd[3] = %d,cmd[4] = %d. \n",cmd[0],cmd[1],cmd[2],cmd[3],cmd[4]);
					...
					//For check 0x53
					if( i2c_himax_read(touch_i2c, 0x53, cmd, 5, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return -1;
					}
					printk("Himax check 0x53 : cmd[0] = %d,cmd[1] = %d,cmd[2] = %d,cmd[3] = %d,cmd[4] = %d. \n",cmd[0],cmd[1],cmd[2],cmd[3],cmd[4]);
	...
	//----[ ESD function]-----------------------------------------------------------------------------------start
		int himax_hang_shaking(void)    //0:Running, 1:Stop, 2:I2C Fail
		{
			...
			msleep(500); //Must more than 1 frame
	...
	//----[HX_FW_UPDATE_BY_I_FILE]--------------------------------------------------------------------------start
				...
				for (i = 0, prePage = 0; i < FileLength; i++)
				{
					...
					if (last_byte == 1)
					{
						...
						if (i == (FileLength - 1))
						{
							...
							//Shouchung modify, modify condition to let i2c fail result will become fail in checksum
							printk("[Himax] checksumResult in fts_ctpm_fw_upgrade_with_i_file = %d", checksumResult);
		...
		static void i_update_func(void)
		{
			...
			uint8_t checksumResult = himax_calculateChecksum(ImageBuffer, fullFileLength);

			printk("[himax]update_flag = %d, checksumResult = %d, if checksumResult = 0 means Fail.\n",update_flag, checksumResult);

			//Shouchung modify, modify condition to let i2c fail result will become fail in checksum
			if (update_flag || checksumResult != 1 )
			//Shouchung end
			{
			...
			{
				printk("[himax] Do not have to update\n");
				i_update_func_success = true;
			}
		}
...
//=============================================================================================================
	...
	//----[HX_TP_PROC_DEBUG_LEVEL]---------------------------------------------------------------------------start
		...
		//Shouchung end
		{
			...
			{
				debug_level_cmd = messages[0];
				if(himax_read_FW_ver(true) != 0)
				{
					printk(KERN_ERR "[TP] %s: debug_level command = 'v' , Read Fw Version Failed.\n", __func__);
				}
				//Shouchung modify, initial porting touch in TF103CG
				//return len;
				return count;
	...
	//----[HX_TP_PROC_FLASH_DUMP]----------------------------------------------------------------------------start
		...
		// For Read Firmware Checksum Start
		static ssize_t himax_get_firmware_checksum(struct device *dev, struct device_attribute *attr, char *buf)
		{
			//Shouchung modify, let get firmware checksum can work correctly
			/*const char buf0[5] = {'w',':','x','8','2'};
			const char buf1[21] = {'w',':','x','E','D',':','x','0','0',':','x','0','4',':','x','0','A',':','x','0','4'};
			const char buf2[17] = {'w',':','x','4','3',':','x','0','1',':','x','0','0',':','x','0','2'};
			const char buf3[9] = {'w',':','x','D','2',':','x','0','5'};
			const char buf4[9] = {'w',':','x','5','3',':','x','0','1'};
			const char buf5[5] = {'r',':','x','A','D'};
			const char buf6[1] = {'1'};

			himax_register_write(dev, attr, buf0, sizeof(buf0));
			himax_register_write(dev, attr, buf1, sizeof(buf1));
			himax_register_write(dev, attr, buf2, sizeof(buf2));
			himax_register_write(dev, attr, buf3, sizeof(buf3));
			msleep(100);
			himax_register_write(dev, attr, buf4, sizeof(buf4));
			himax_register_write(dev, attr, buf5, sizeof(buf5));
			#ifdef HX_TP_PROC_RESET
			himax_reset_write(dev, attr, buf6, sizeof(buf6));
			#endif
			...
			if (register_command == 0xad)
			{
				printk(KERN_INFO "Himax register_command = %x \n",register_command);
			}
			...
			if(IC_TYPE == HX_85XX_E_SERIES_PWON)
			{
				...
				for (loop_i = 0; loop_i < 4; loop_i++)
				{
					if (loop_i == 3)
					{
						ret += sprintf(buf + ret, "0x%2.2X", cmd[loop_i]);
					}
					else
					{
						ret += sprintf(buf + ret, "0x%2.2X ", cmd[loop_i]);
		...
		static DEVICE_ATTR(touch_checksum, (S_IRUGO), himax_get_firmware_checksum, NULL);
		// For Read Firmware Checksum End
	...
	static ssize_t himax_touch_switch_name(struct switch_dev *sdev, char *buf)
	{
		if(himax_read_FW_ver(true) != 0)
		{
			printk(KERN_ERR "[TP] %s: Read Fw Version Failed.\n", __func__);
		}
		mdelay(1000);
		int count = 0;
	...
	static struct attribute *himax_attr[] =
	{
		...
		&dev_attr_touch_checksum.attr,	// for Read Firmware Checksum
	...
	static int himax_touch_sysfs_init(void)
	{
		...
		// for Read Firmware Checksum Start
		ret = sysfs_create_file(android_touch_kobj, &dev_attr_touch_checksum.attr);
		if (ret)
		{
			printk(KERN_ERR "[Himax]TOUCH_ERR: sysfs_create_file dev_attr_touch_checksum failed\n");
			return ret;
		}
		// for Read Firmware Checksum End
	...
	static void himax_touch_sysfs_deinit(void)
	{
		...
		sysfs_remove_file(android_touch_kobj, &dev_attr_touch_checksum.attr);	// for Read Firmware Checksum
			...
			for(i=0; i<HX_MAX_PT; i++)
			{
				if(buf[4*i] != 0xFF)
				{
					...
					if((x <= x_res) && (y <= y_res))
					{
						...
						#ifdef HX_PORTING_DEB_MSG
						printk("[HIMAX MSG]%s Touch DOWN x = %d, y = %d, area = %d, press = %d.\n",__func__, x, y, area, press);
						#endif

						if (Flag_print_point == 1)
						{
							printk("[HIMAX MSG]%s x = %d, y = %d, area = %d, press = %d.\n",__func__, x, y, area, press);
						}
...
//=============================================================================================================
	...
	//----[HX_ESD_WORKAROUND]-------------------------------------------------------------------------------end
			...
			// *.i FW update
			//*********************************************************************************************************
			while (i_update_func_success == false && i_update_count <= 3)
			{
				printk("[HIMAX MSG]%s : i_update_func_success = %d, i_update_count = %d. \n",__func__,i_update_func_success,i_update_count);

				if (i_Check_FW_Version_func() == false)
				{
					printk(KERN_ERR "[HIMAX TP ERROR] %s: i Check FW Version failed\n", __func__);
				}
				#ifdef HX_RST_PIN_FUNC
					himax_HW_reset();
				#endif
				...
				{
					i_update_func();
					i_update_count = i_update_count + 1;
				}
				#endif
			}
